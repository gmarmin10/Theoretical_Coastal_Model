...
for n in it_Nt:
    ...
    Pnut[np.isnan(Pnut)]=0
    Nnut[np.isnan(Nnut)]=0
    Pbio[np.isnan(Pbio)]=0
    Nbio[np.isnan(Nbio)]=0
...
x=x*1e-3
plt.plot(x,Pnut,color='orange')
plt.xlabel('horizontal distance (km)')
plt.ylabel('PO4 (uM)')
plt.subplot(2,2,2)
plt.plot(x,Nnut,color='green')
plt.xlabel('horizontal distance (km)')
plt.ylabel('NO3 (uM)')
plt.subplot(2,2,3)
plt.plot(x,Pbio,color='red')
plt.xlabel('horizontal distance (km)')
plt.ylabel('Phyto P (uM)')
plt.subplot(2,2,4)
plt.plot(x,Nbio,color='blue')
plt.xlabel('horizontal distance (km)')
plt.ylabel('Phyto N (uM)') 
plt.tight_layout()
plt.savefig('Nutrient_Concentrations_weird.png')
plt.show()
